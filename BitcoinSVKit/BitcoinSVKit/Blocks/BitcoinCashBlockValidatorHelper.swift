...
class BitcoinSVBlockValidatorHelper: IBitcoinSVBlockValidatorHelper {
    private let medianTimeSpan = 11
    private let bitcoinSVStorage: IBitcoinSVStorage
    private let coreBlockValidatorHelper: IBlockValidatorHelperWrapper

    init(storage: IBitcoinSVStorage, coreBlockValidatorHelper: IBlockValidatorHelperWrapper) {
        bitcoinSVStorage = storage
    ...
    func medianTimePast(block: Block) -> Int {
        ...
        var median = bitcoinSVStorage.timestamps(from: startIndex, to: block.height, ascending: true)
